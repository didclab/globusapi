...
public class CustomTokenResponse extends GenericJson {
    ...
    private Calendar recievedTime = Calendar.getInstance();

    public CustomTokenResponse() {
        this.recievedTime.add(13, 172800);
    }

    public final String getTransferAccessToken() {
        for(int i = 0; i < this.other_token.size(); ++i) {
            if ("transfer.api.globus.org".equals(((LinkedHashMap)this.other_token.get(i)).get("resource_server"))) {
                return (String)((LinkedHashMap)this.other_token.get(i)).get("access_token");
            }
        }
    ...
    public final Date getExpiredTime() {
        for(int i = 0; i < this.other_token.size(); ++i) {
            if ("transfer.api.globus.org".equals(((LinkedHashMap)this.other_token.get(i)).get("resource_server"))) {
                return this.recievedTime.getTime();
            }
        }
